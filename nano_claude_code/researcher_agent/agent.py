...
from researcher_agent.tools import fetch_mcp_tools

async def create_researcher_agent(model, checkpointer):
    mcp_tools = await fetch_mcp_tools()
    tools = [read_file] + mcp_tools
    system_prompt = "你是一个技术调研专家。请使用 MCP 工具深入搜索 LangChain 相关文档，为用户提供最新的 API 用法和代码示例。"
